module.exports = async (MAIN, object) => {
  return new Promise(async function(resolve, reject) {
    ...
    if(object.pokemon_id){ data.pokemon_name = MAIN.masterfile.pokemon[object.pokemon_id].name; }
    if(object.form && object.form > 0){
      if(!MAIN.masterfile.pokemon[object.pokemon_id].forms[object.form]){
        ...
        data.form = '['+MAIN.masterfile.pokemon[object.pokemon_id].forms[object.form].name+'] ';
      }
    }
    ...
    if(object.move_1){
      if(MAIN.masterfile.moves[object.move_1]){
        data.move_1 = MAIN.masterfile.moves[object.move_1].name;
      } else{
        console.error('[masterfile.json] Missing move info for move '+object.move_1);
        data.move_1 = object.move_1;
      }
    }
    if(object.move_2){
      if(MAIN.masterfile.moves[object.move_2]){
        data.move_2 = MAIN.masterfile.moves[object.move_2].name;
      } else{
        console.error('[masterfile.json] Missing move info for move '+object.move_2);
        data.move_2 = object.move_2;
      }
    }

    // IF LURE NAME
    if(object.lure_id){ data.lure_type = MAIN.Get_Lure(MAIN, object.lure_id); }
